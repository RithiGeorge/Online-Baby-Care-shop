...
<div  id="hidden_div" style="display:none;background-color:red;">
<tr><td id="hidden_div1" style="display:none;"> Starting Date</td><td><input id="hidden_div3"  style="display:none;" type="date" name="sdate"   class="form-control"></td></tr>
<tr><td id="hidden_div2" style="display:none;"> Ending Date</td><td><input id="hidden_div4"  style="display:none;" type="date" name="edate"   class="form-control"></td></tr>
</div>

<tr><td colspan="2" align="center"><input type="submit" name="search" value="search" class="btn btn-success">
...
<tr>
  ...
  <?php
  $ui="select * from tbl_customer where status='1'";
  $qr=mysqli_query($conn,$ui);
  while($row=mysqli_fetch_array($qr))
  {
          echo '<tr><td>'.$row['fname'].' ' .$row['lname'].'</td><td>'.$row['phone'].'</td><td>'.$row['email'].' '. $row['lname'].'</td><td>'.$row['housename'].'</td><td>'.$row['city'].'</td><td>'.$row['district'].'</td><td>'.$row['pincode'].'</td></tr>'; 
    ...
    <tr>
      ...
      <?php
      $ui="select * from tbl_staff where status='1'";
      $qr=mysqli_query($conn,$ui);
      while($row=mysqli_fetch_array($qr))
      {
    
   
              echo '<tr><td>'.$row['fname'].' ' .$row['lname'].'</td><td>'.$row['phone'].'</td><td>'.$row['email'].' '. $row['lname'].'</td><td>'.$row['houseno'].'</td><td>'.$row['street'].'</td><td>'.$row['district'].'</td><td>'.$row['pincode'].'</td></tr>'; 
        ...
        <tr>
          ...
          <?php
          $ui="select * from tbl_supplier where supstatus='1'";
          $qr=mysqli_query($conn,$ui);
          while($row=mysqli_fetch_array($qr))
          {
...
<!-- <table border='3'   style="float:right" >
  ...
  <input type="submit" name="pr" value="Print" style="margin-left:500px;"  class="btn btn-primary">
</form> -->
  <br>    <br>    <br>    <br>    <br>   
</div>  
