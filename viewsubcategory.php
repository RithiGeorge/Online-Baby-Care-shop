...
<div id="demo">
    ...
    $masterid=$_GET['bmid'];
    $date=$_GET['date'];
    $a="select * from tbl_subcat where catid='$id'";
    $r=mysqli_query($conn,$a);
    while($row=mysqli_fetch_array($r))
    {

   echo' <a href="viewitems.php?subid='.$row['scid']. '&bmid='.$masterid.'&date='.$date.'" class="btn btn-success" style="width: 200px;height:60px;margin-top: 60px;font-size: 30px;text-align: center;">' .$row['category'].'</a>';
